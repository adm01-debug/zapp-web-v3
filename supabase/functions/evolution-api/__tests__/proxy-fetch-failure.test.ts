 ...
 *
 * Note: sanitizeOps/Resources are disabled because, on a synchronous fetch
 * rejection, the internal `setTimeout` used for AbortController guarding is
 * not cleared. That's a known pre-existing behavior in the proxy and not
 * what these tests aim to validate — we focus on the response contract.
 */
