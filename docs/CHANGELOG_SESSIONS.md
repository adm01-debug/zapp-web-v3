...
### Rodada 4 (continuação) — idempotência do lote completo 040000-200000 concluída manualmente

A checagem de idempotência das 4 migrations que faltavam (`170000`, `180000`, `195000`, `200000` — o agente da rodada 4 morreu por rate-limit semanal em `160000`) foi concluída manualmente nesta sessão, já após o reset do limite (06/09). Confirmado antes: `supabase_db_query` NÃO mantém transação entre chamadas HTTP separadas (`BEGIN` numa chamada e `SELECT txid_current()` na próxima retornam `txid` diferentes) — então `BEGIN;`/`ROLLBACK;` só funcionam se estiverem na MESMA chamada que o corpo da migration. Validado isso com um teste inofensivo (`CREATE TABLE` + `ROLLBACK` na mesma chamada → tabela não existe depois), e só então rodei o SQL literal das 4 migrations dentro de `BEGIN; ...; ROLLBACK;` numa única chamada cada. As 4 são só `CREATE OR REPLACE FUNCTION` (confirmado por análise estática antes de rodar — sem `ALTER TABLE`/`CREATE TYPE`/`ADD CONSTRAINT` sem guarda) e rodaram sem erro. **Lote completo 040000-200000 agora 100% confirmado idempotente**, sem persistir nada (nunca usado `supabase_db_transaction`, sempre `BEGIN`/`ROLLBACK` explícito na mesma chamada).

### Rodada 5 — modelo de posse (admin/supervisor ou dono do registro) em 9 RPCs de CRM (PR #1538)

Pendência aberta desde `20260902170000_harden_unguarded_crm_rpcs.sql`: `rpc_delete_message`, `rpc_change_deal_stage`/`rpc_move_deal`/`rpc_upsert_deal`, `rpc_purge_contact_intelligence`, `bulk_update_lead_status`, `grant_lgpd_consent`/`revoke_lgpd_consent`, `rpc_complete_task` já exigiam `fn_require_app_user()` (sessão de app válida), mas nenhuma verificava se o registro-alvo pertencia ao chamador — qualquer agente autenticado podia apagar mensagem de outro, mudar estágio de negócio alheio, purgar inteligência de IA de qualquer contato, mudar lead status/consentimento LGPD/completar tarefa de registro de outro agente.

**Decisão de negócio do dono**, dada explicitamente via pergunta com 3 opções (manter aberto / restringir a admin-ou-dono / híbrido só para dados sensíveis): **"Restringir a admin/supervisor ou dono do registro"**.

Modelo implementado (`20260906120000_harden_ownership_crm_rpcs.sql`): cada uma das 9 RPCs agora exige, além de `fn_require_app_user()`, que o chamador real (`auth.uid()`, nunca parâmetro) seja `zapp.is_admin_or_supervisor()` OU o `assigned_to` do registro-alvo (confirmado ao vivo via `information_schema`: `evolution_contacts`/`evolution_deals`/`evolution_tasks` têm `assigned_to`; `evolution_messages`/`contact_intelligence` herdam o dono do contato relacionado). Registro sem dono (`assigned_to IS NULL`) só pode ser mexido por admin/supervisor. `rpc_upsert_deal` só restringe o ramo UPDATE — criar negócio novo continua livre. `bulk_update_lead_status` filtra posse direto no `WHERE` do UPDATE (atualização parcial silenciosa, visível via `requested` vs `updated` no retorno) em vez de bloquear a chamada inteira.

**Efeito colateral aceito conscientemente**: único caller real de `grant_lgpd_consent`/`revoke_lgpd_consent` é `ContactConsentManager.tsx`, que hoje abre para qualquer contato visível ao agente, não só os atribuídos a ele — um agente gerenciando consentimento de um contato alheio agora é bloqueado (a menos que seja admin/supervisor), trade-off explicado ao dono antes da decisão. As outras 6 RPCs não têm nenhum caller real no repo hoje, então restringir não muda comportamento observável.

Testado ao vivo: migration aplicada direto em produção via `supabase_db_query`, validada com 15/15 asserções dentro de uma transação (simulando `auth.uid()` via `set_config('request.jwt.claim.sub', ...)`, revertendo toda mutação de teste antes do commit) — 6/6 bloqueios corretos de não-dono/não-admin, 2/2 bypass corretos de admin/supervisor, 7/7 acessos corretos do dono do registro. Confirmado pós-teste: nenhum dado real de produção alterado. Pendência aberta: efeito colateral em `grant_lgpd_consent`/`revoke_lgpd_consent` precisa validação em uso real pós-deploy (sem sessão de usuário real disponível nesta sessão).

Mergeado como PR #1538 em 2026-09-06 (commit `a8c50b85c7`).

### Rodada 6 — race condition no Sicoob bridge corrigida com advisory lock (PR #1539)

Fecha a pendência de design registrada desde a rodada 3 (`20260902190000`/`20260902200000`): duas chamadas verdadeiramente concorrentes de `zapp.fn_sicoob_bridge_ingest_message` para o mesmo remetente (mesmo `sicoob_user_id` + `singular_id`) podiam passar pela checagem de idempotência (`SELECT` em `evo.evolution_messages`) e pelo lookup de `sicoob_contact_mapping` antes de qualquer uma das duas ter inserido — causando (1) contato duplicado quando o remetente é novo, e (2) falso `idempotent=false` em mensagem duplicada (o `INSERT` passa pela view com `ON CONFLICT DO NOTHING`, que não lança `unique_violation`; a segunda chamada concorrente recebe `message_id=NULL` para uma mensagem que já foi persistida pela primeira).

Fix (`20260906130000_fix_sicoob_bridge_concurrency_advisory_lock.sql`): `pg_advisory_xact_lock(hashtextextended(sicoob_user_id || '|' || singular_id, 42))` logo no início da função. Serializa todas as chamadas concorrentes do mesmo remetente; remetentes diferentes continuam paralelos. Lock transacional (`_xact_`) — libera sozinho no fim da chamada, sem risco de vazamento mesmo com exceção no meio.

Testado ao vivo: aplicado direto em produção via `supabase_db_query`, validado com 5/5 casos sequenciais sem regressão (envio novo, replay imediato, replay pós soft-delete, não confundir mensagem de outra instância, `singular_id` nulo não quebra o lock) — zero resíduo de dados sintéticos. **Limitação declarada**: não foi possível provar o bloqueio cross-sessão ao vivo — `supabase_db_query` abre uma conexão nova a cada chamada e não sustenta duas transações simultâneas de verdade, então a garantia de bloqueio em si fica sem prova direta nesta sessão (só a não regressão do comportamento sequencial). `pg_advisory_xact_lock` é primitivo padrão e bem documentado do Postgres para exatamente este padrão de "check-then-act".

**Nota de processo**: o merge de `main` para a branch desta PR feito por uma sessão concorrente (commit `2473ac40dd`, "resolve conflito CHANGELOG") mesclou uma `main` desatualizada (`cdbef314ab`, anterior ao merge do PR #1538) em vez da `main` corrente (`a8c50b85c7`) — o PR ficou `mergeable_state: dirty` contra a `main` real. Corrigido nesta sessão com um novo merge de `main` atual, resolvendo o conflito real em `sprint1-security-hardening.test.ts` (mantendo as descriptions da Rodada 5 e da Rodada 6, na ordem) e recuperando estas duas entradas do changelog (Rodada 5 e Rodada 6), que o merge desatualizado havia descartado silenciosamente sem deixar marcador de conflito.
