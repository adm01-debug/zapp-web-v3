...
Migration: `supabase/migrations/20260906120000_harden_ownership_crm_rpcs.sql` (aplicada direto em produção via `supabase_db_query`; PR [#1538](https://github.com/adm01-debug/Zapp_Web_V3/pull/1538)).

9 RPCs agora exigem `is_admin_or_supervisor()` OU `assigned_to = auth.uid()::text` do registro-alvo (dono por entidade: `evolution_contacts`/`evolution_deals`/`evolution_tasks` via `assigned_to`; `evolution_messages`/`contact_intelligence` herdam o dono do contato relacionado). Testado ao vivo: 15/15 asserções (6 bloqueios de não-dono/não-admin, 2 bypass de admin, 7 acessos de dono), zero resíduo em produção. Efeito colateral aceito conscientemente: `grant_lgpd_consent`/`revoke_lgpd_consent` (único par com caller real, `ContactConsentManager.tsx`) agora bloqueiam agente não-admin gerenciando consentimento de contato não atribuído a ele — precisa validação em uso real pós-deploy.

### Rodada 6 — serialização da race condition do Sicoob bridge (advisory lock)

Pendência de design registrada desde a rodada 3 (`20260902190000`/`20260902200000`): duas chamadas verdadeiramente concorrentes de `fn_sicoob_bridge_ingest_message` para o MESMO remetente (mesmo `sicoob_user_id`+`singular_id`) podiam passar pela checagem de idempotência e pelo lookup de `sicoob_contact_mapping` antes de qualquer uma inserir — causando contato duplicado (remetente novo, ambas caem no ramo de criação) ou um falso `idempotent=false` em mensagem duplicada (o `INSERT` passa pela view com `ON CONFLICT DO NOTHING`, que devolve `id` `NULL` em vez de lançar `unique_violation` — o `EXCEPTION WHEN unique_violation` já existente nunca pega essa corrida).

Migration: `supabase/migrations/20260906130000_fix_sicoob_bridge_concurrency_advisory_lock.sql`. Fix: `pg_advisory_xact_lock(hashtextextended(sicoob_user_id || '|' || singular_id, 42))` logo no início da função — serializa todas as chamadas concorrentes do MESMO remetente (cobre os dois problemas, já que ambos só ocorrem quando a identidade colide); remetentes diferentes continuam paralelos. Lock transacional (`_xact_`), libera sozinho no fim da chamada, sem risco de vazamento mesmo se a função levantar exceção no meio.

**Limitação de validação desta sessão, declarada explicitamente**: as ferramentas de banco disponíveis (`supabase_db_query`) abrem uma conexão nova a cada chamada e não sustentam duas transações simultâneas de verdade — não foi possível reproduzir a corrida real (2 conexões concorrentes de fato) para provar o bloqueio cross-sessão ao vivo. `pg_advisory_xact_lock` é primitivo padrão e bem documentado do Postgres para exatamente este padrão de "check-then-act"; a mudança foi validada quanto a **não regredir** o comportamento sequencial já testado antes (5/5 casos: envio novo, replay imediato, replay pós soft-delete, não confundir mensagem de outra instância, `singular_id` nulo não quebra o lock), zero resíduo em produção — mas não quanto ao bloqueio cross-sessão em si, que fica sem prova ao vivo nesta sessão.
