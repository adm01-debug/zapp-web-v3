    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          className="mt-3 space-y-3 rounded-lg bg-muted/50 p-3"
        >
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">Tipo</label>
            <div className="flex flex-wrap gap-2">
              <Toggle
                pressed={activeTypes.has('action')}
                onPressedChange={() => onToggleType('action')}
                size="sm"
              ...
              <Toggle
                ...
                onPressedChange={() => onToggleType('message')}
                size="sm"
              ...
              <Toggle
                ...
                onPressedChange={() => onToggleType('transcription')}
                size="sm"
                className="gap-1.5 data-[state=on]:bg-warning data-[state=on]:text-primary-foreground"
              >
                <Mic className="h-3.5 w-3.5" /> Transcrições
              </Toggle>
              <Toggle
                pressed={activeTypes.has('contact')}
                onPressedChange={() => onToggleType('contact')}
                size="sm"
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              <Toggle
                pressed={mediaTypeFilter === 'all'}
                onPressedChange={() => onSetMediaTypeFilter('all')}
                size="sm"
                className="gap-1.5 data-[state=on]:bg-muted-foreground/20 data-[state=on]:text-foreground"
              >
                <MessageSquare className="h-3.5 w-3.5" /> Todos
              </Toggle>
              <Toggle
                pressed={mediaTypeFilter === 'text'}
                onPressedChange={() =>
                  onSetMediaTypeFilter(mediaTypeFilter === 'text' ? 'all' : 'text')
                }
                size="sm"
                className="gap-1.5 data-[state=on]:bg-primary data-[state=on]:text-primary-foreground"
              >
              ...
              <Toggle
                pressed={mediaTypeFilter === 'image'}
                onPressedChange={() =>
                  onSetMediaTypeFilter(mediaTypeFilter === 'image' ? 'all' : 'image')
                }
                size="sm"
                className="gap-1.5 data-[state=on]:bg-success data-[state=on]:text-primary-foreground"
              >
                <Image className="h-3.5 w-3.5" /> Imagens
              </Toggle>
              <Toggle
                pressed={mediaTypeFilter === 'video'}
                onPressedChange={() =>
                  onSetMediaTypeFilter(mediaTypeFilter === 'video' ? 'all' : 'video')
                }
                size="sm"
                className="gap-1.5 data-[state=on]:bg-info data-[state=on]:text-primary-foreground"
              >
                <Video className="h-3.5 w-3.5" /> Vídeos
              </Toggle>
              <Toggle
                pressed={mediaTypeFilter === 'audio'}
                onPressedChange={() =>
                  onSetMediaTypeFilter(mediaTypeFilter === 'audio' ? 'all' : 'audio')
                }
                size="sm"
                className="gap-1.5 data-[state=on]:bg-warning data-[state=on]:text-primary-foreground"
              >
              ...
              <Toggle
                pressed={mediaTypeFilter === 'document'}
                onPressedChange={() =>
                  onSetMediaTypeFilter(mediaTypeFilter === 'document' ? 'all' : 'document')
                }
                size="sm"
                className="gap-1.5 data-[state=on]:bg-secondary data-[state=on]:text-secondary-foreground"
              >
              ...
              <Toggle
                pressed={mediaTypeFilter === 'link'}
                onPressedChange={() =>
                  onSetMediaTypeFilter(mediaTypeFilter === 'link' ? 'all' : 'link')
                }
                size="sm"
                className="gap-1.5 data-[state=on]:bg-accent data-[state=on]:text-accent-foreground"
              >
