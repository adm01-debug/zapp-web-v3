...
export function useChatMediaSending(contactId: string, contactPhone: string | undefined) {
  ...
  const resolveInstance = useCallback(async (): Promise<string> => {
    ...
    try {
      ...
      const { data: evoContact, error: _evoContactErr } = await (supabase as any)
      ...
      if (evoContact?.whatsapp_connection_id) {
        ...
        const { data: contact, error: _contactErr } = await supabase
          .from('contacts')
          .select('whatsapp_connection_id')
          .eq('id', contactId)
          .maybeSingle();
      ...
      const { data: fallbackConn, error: _fallbackConnErr } = await supabase
        .from('whatsapp_connections')
        .select('instance_id')
        .eq('status', 'connected')
        .limit(1)
