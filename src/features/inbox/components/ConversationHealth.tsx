...
export function ConversationHealth({
  ...
  const metrics = useMemo(() => {
    ...
    const failedMessages = agentMessages.filter(
    ...
    if (contactMessages.length === 0 && agentMessages.length === 0) {
      return {
        ...
        color: 'text-info',
        icon: Zap,
        details: { responseTime: 0, msgPerMin: 0, failureRate: 0 },
      };
    }
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <motion.div
          ...
          className={cn('flex cursor-help items-center gap-1.5', className)}
        >
          <div
            className={cn(
              'rounded-full border border-border/20 bg-background/50 p-1 shadow-sm',
              metrics.color
            )}
          >
          ...
          <div className="flex flex-col -space-y-0.5">
            <span className="text-[10px] font-bold uppercase tracking-tighter opacity-50">SLA</span>
            <span className={cn('text-xs font-bold leading-none', metrics.color)}>
              {metrics.label}
            </span>
          </div>
        </motion.div>
      </TooltipTrigger>
      <TooltipContent side="bottom" className="w-64 border-border/50 bg-card p-3 shadow-xl">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold">Saúde da Conversa</span>
            <Badge variant="outline" className={cn('text-[10px] font-bold', metrics.color)}>
              {metrics.score}%
            </Badge>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 border-t border-border/20 pt-1">
            ...
            <div className="flex flex-col">
              ...
              <span
                className={cn(
                  'text-xs',
                  metrics.details.failureRate > 10 ? 'text-destructive' : 'text-foreground'
                )}
              >
          ...
          <p className="text-[10px] italic leading-tight text-muted-foreground">
            Métricas baseadas em SLAs configuráveis. Um score baixo pode indicar gargalos no
            atendimento ou falhas técnicas.
          </p>
        </div>
      </TooltipContent>
    </Tooltip>
