...
export function useChatPanelHandlers(opts: UseChatPanelHandlersOptions) {
      ...
      try {
        ...
        if (isWhisperRef.current) {
          if (attachments && attachments.length > 0) {
            toast({
              title: 'Aviso',
              description: 'Arquivos não são suportados em modo sussurro no momento.',
              variant: 'destructive',
            });
          }
    ...
    (command: SlashCommand, subCommand?: string) => {
      ...
      switch (command.id) {
        ...
        case 'template':
          ...
          break;
        case 'note':
          toast({ title: '📝 Nota Privada', description: 'Funcionalidade de notas será aberta.' });
          break;
        case 'tag':
          toast({
            title: subCommand === 'add' ? '🏷️ Adicionar Tag' : '🏷️ Remover Tag',
            description:
              subCommand === 'add'
  ...
  const handleAudioSend = useCallback(
    async (audioBlob: Blob, onSendAudio?: (blob: Blob) => Promise<void>) => {
      if (!onSendAudio) {
        toast({
          title: 'Erro',
          description: 'Envio de áudio não configurado.',
          variant: 'destructive',
        });
  ...
  const handleAudioVoiceChange = useCallback(async (messageId: string, newBlob: Blob) => {
    try {
      ...
      const _url = URL.createObjectURL(newBlob);
      toast({
        title: 'Voz alterada!',
