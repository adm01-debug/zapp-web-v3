...
export const ChatMessagesArea = memo(
  forwardRef<ChatMessagesAreaRef, ChatMessagesAreaProps>(
    (
      ...
      useEffect(() => {
        if (
          prevContactJidRef.current !== undefined &&
        ...
        scrollToMessage: (messageId: string): boolean => {
          ...
          requestAnimationFrame(() => {
            try {
              ...
              /* noop em ambientes sem scrollIntoView */
            }
          });
      ...
      const groupedMessages = useMemo(() => {
        return displayedMessages.reduce(
          (groups, message) => {
      ...
      return (
        <div
          ...
          className="scrollbar-none relative min-h-0 min-w-0 flex-1 space-y-4 overflow-y-auto bg-background/20 px-4 py-6 transition-all duration-500 focus-visible:outline-none md:px-24"
          tabIndex={0}
        >
          <ChatWatermark />

          {isLoading && (
            <div className="space-y-8 pt-10 duration-700 animate-in fade-in">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className={cn('flex flex-col gap-3', i % 2 === 0 ? 'items-start' : 'items-end')}
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div
                      className={cn(
                        'relative h-16 overflow-hidden rounded-2xl bg-muted/30',
                        i % 2 === 0 ? 'w-64 rounded-tl-none' : 'w-56 rounded-tr-none bg-primary/5'
                      )}
                    >
                      <div className="absolute inset-0 -translate-x-full animate-shimmer bg-gradient-to-r from-transparent via-background/10 to-transparent" />
                    </div>
                  </div>
          ...
          {messages.length > 0 && (
            <div className="pointer-events-none sticky top-0 z-20 -mt-2 mb-2 flex flex-col items-center gap-1.5">
          ...
          {messages.length > 0 && displayedMessages.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-4 py-20 text-center duration-300 animate-in fade-in zoom-in">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-muted">
                {statusFilter.has('failed' as any) ? (
                  <Ban className="h-8 w-8 text-destructive opacity-50" />
                ) : (
                  <Navigation2 className="h-8 w-8 text-muted-foreground opacity-50" />
                )}
              </div>
          ...
          <AnimatePresence>
            {showScrollBottom && (
              <motion.div
                ...
                className="fixed bottom-24 right-8 z-40 md:right-32"
              >
                <Button
                  variant="secondary"
                  size="icon"
                  className="h-10 w-10 rounded-full border border-border/50 bg-background/80 shadow-lg backdrop-blur-sm transition-all duration-300 hover:bg-background"
                  onClick={() => {
                    const container = scrollContainerRef.current;
                    if (container)
      ...
      */}
          <div className="mb-8 flex flex-col items-center gap-4 pt-4">
            <motion.div
              ...
              className="max-w-sm rounded-2xl border border-border/30 bg-card/50 p-6 text-center shadow-sm backdrop-blur-sm"
            >
              ...
              <p className="text-[12px] leading-relaxed text-muted-foreground">
                As mensagens e chamadas são protegidas com criptografia. Ninguém fora desta conversa
                pode lê-las.
              </p>
            </motion.div>
            ...
            {onLoadOlder ? (
              <div className="flex justify-center" aria-live="polite">
                {loadingOlder && (
                  <div className="flex items-center gap-2 rounded-full border border-border/30 bg-muted/50 px-3 py-1.5 text-[11px] text-muted-foreground backdrop-blur-sm">
                ...
                {!loadingOlder && !loadCancelled && !hasMoreOlder && messages.length > 0 && (
                  <div className="flex items-center gap-1.5 rounded-full bg-muted/30 px-3 py-1 text-[11px] text-muted-foreground/60">
            ...
            <div key={dateKey}>
              <div className="my-5 flex justify-center">
                <motion.span
                  ...
                  className="rounded-full border border-border/30 bg-muted/50 px-4 py-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/80 shadow-sm backdrop-blur-sm"
                >
...
function LoadCancelledBadge({
  ...
  return (
    <div
      ...
      className="flex items-center gap-2 rounded-full border border-border/30 bg-muted/40 py-1 pl-3 pr-1 text-[11px] text-muted-foreground backdrop-blur-sm"
    >
      ...
      <button
        ...
        className="ml-1 inline-flex items-center gap-1 rounded-full border border-border/40 bg-background/80 px-2 py-0.5 text-[11px] font-medium text-foreground transition-colors hover:bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
