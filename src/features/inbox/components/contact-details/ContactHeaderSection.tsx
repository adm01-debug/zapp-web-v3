...
import { ImagePreview } from '@/features/inbox/components/ImagePreview';
  ...
  return (
    <>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        className="p-4 flex flex-col items-center text-center border-b border-border/10 bg-background dark:bg-background">
          ...
          <div className="relative inline-block">
            ...
            {crmCompany?.logo_url && (
              <img src={crmCompany.logo_url} alt={crmCompany.nome_fantasia || ''}
                className="absolute -top-1 -left-1 w-8 h-8 rounded-md object-contain bg-background border border-border/10 ring-2 ring-background" />
            )}
          </div>
        </div>
