...
export function EnhancedProgressiveDisclosure({
  ...
  const totalWidgets = useMemo(
    () => sections.reduce((a, s) => a + s.widgets.length, 0),
    [sections]
  );
  const visibleWidgets = useMemo(
    () => sections.filter((s) => openSections.has(s.id)).reduce((a, s) => a + s.widgets.length, 0),
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          Mostrando <span className="font-medium text-foreground">{visibleWidgets}</span> de{' '}
        ...
        {sections.map((section) => {
          ...
          return (
            ...
            >
              ...
              <CollapsibleContent>
                <AnimatePresence>
                  {isOpen && <WidgetGrid widgets={section.widgets} renderWidget={renderWidget} />}
                </AnimatePresence>
              </CollapsibleContent>
            </Collapsible>
          );
