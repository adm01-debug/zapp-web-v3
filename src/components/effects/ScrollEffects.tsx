...
export function TextReveal({ text, className, delay: _delay = 0 }: TextRevealProps) {
  const ref = useRef<HTMLDivElement>(null);
...
export function Counter({
  ...
  return (
    ...
    >
      {prefix}
      <motion.span>{springValue.get().toFixed(0)}</motion.span>
      {suffix}
    </motion.span>
  );
}
...
export function GradientText({
  ...
  return (
    <motion.span
      className={cn('bg-[length:200%_auto] bg-clip-text text-transparent', className)}
