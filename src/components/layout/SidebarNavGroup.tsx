...
export function SidebarNavGroup({ label, icon: GroupIcon, items, currentView, onViewChange, defaultOpen = false, collapsed = true, onToggleFavorite, isFavorite, badgeMap }: SidebarNavGroupProps) {
  ...
  const triggerButton = (
    <button
      ...
      className={cn(
        'rounded-xl flex items-center transition-all duration-300 group/trigger',
        collapsed ? 'w-full h-[34px] justify-center gap-0.5' : 'w-full h-[34px] px-3 gap-2 hover:bg-muted/15',
        hasActiveItem
