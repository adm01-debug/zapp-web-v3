...
interface RolePermission {
  role: 'dev' | 'admin' | 'manager' | 'supervisor' | 'agent';
...
export function usePermissions() {
  ...
  const fetchRolePermissions = useCallback(async () => {
    ...
    if (!error && data) {
      const mapped = data.map(rp => ({
        role: rp.role as 'dev' | 'admin' | 'manager' | 'supervisor' | 'agent',
  ...
  const addPermissionToRole = useCallback(async (role: 'dev' | 'admin' | 'manager' | 'supervisor' | 'agent', permissionId: string) => {
  ...
  const removePermissionFromRole = useCallback(async (role: 'dev' | 'admin' | 'manager' | 'supervisor' | 'agent', permissionId: string) => {
    const { error } = await supabase
      .from('role_permissions')
      .delete()
      .eq('role', role)
