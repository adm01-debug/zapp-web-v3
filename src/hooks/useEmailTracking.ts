...
export function useEmailTracking() {
  ...
  const createTracking = useCallback(
    async (params: {
      ...
      try {
        ...
        await supabase.from('email_contact_scores').upsert(
          {
            user_id: user.id,
  ...
  const loadStats = useCallback(async (days = 30) => {
    try {
      const { data, error: rpcErr } = await supabase.rpc('rpc_email_tracking_stats', {
        p_days: days,
      });
  ...
  const loadTopContacts = useCallback(async (limit = 10) => {
    try {
      const { data, error: rpcErr } = await supabase.rpc('rpc_email_top_contacts', {
        p_limit: limit,
      });
