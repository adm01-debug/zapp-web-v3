...
export function useContactSearch(options: ContactSearchOptions = {}) {
  const { instanceName = 'wpp2', leadStatus = null, limit = 20, debounceMs = 400 } = options;
      ...
      timerRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (error) {
            ...
            const { data: fallback, error: _fallbackErr } = await (supabase as any)
              .from('evolution_contacts')
              .select(
                'id,remote_jid,phone_number,full_name,push_name,email,company,lead_status,lead_score,tags'
              )
              .is('deleted_at', null)
              .eq('instance_name', instanceName)
              .or(
                `full_name.ilike.%${sanitized}%,push_name.ilike.%${sanitized}%,phone_number.ilike.%${sanitized}%,email.ilike.%${sanitized}%`
              )
              .limit(limit);
