  ...
  const loadMetrics = useCallback(async () => {
    ...
    try {
      ...
      const {
        ...
        error: dbErr,
        requestId,
      } = await safeClient.from('email_daily_metrics', (q) =>
        q
          .select(
            'date, threads_received, threads_replied, avg_first_reply_minutes, sla_met_count, sla_breached_count'
          )
      ...
      const { data: threadsData } = await safeClient.from('email_threads', (q) =>
        q.select('sla_status').eq('account_id', accountId).not('sla_status', 'is', null)
      );
