...
export function useEmailTemplates() {
  ...
  const updateTemplate = useCallback(async (id: string, updates: Partial<CreateTemplateParams>) => {
    const { error: dbErr } = await supabase.from('email_templates').update(updates).eq('id', id);

    if (dbErr) return { success: false, error: dbErr.message };
    setTemplates((prev) =>
      prev.map((t) => (t.id === id ? ({ ...t, ...updates } as EmailTemplate) : t))
    );
    return { success: true };
  }, []);
  ...
  const deleteTemplate = useCallback(async (id: string) => {
    const { error: dbErr } = await supabase.from('email_templates').delete().eq('id', id);
  ...
  const getByShortcut = useCallback(async (shortcut: string): Promise<EmailTemplate | null> => {
    const { data, _error } = await supabase.rpc('rpc_email_template_by_shortcut', {
      p_shortcut: shortcut,
    });
  ...
  const detectShortcut = useCallback(
    (text: string): EmailTemplate | undefined => {
