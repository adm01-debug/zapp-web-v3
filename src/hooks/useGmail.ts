...
export function useGmail(accountId?: string) {
  ...
  const connectGmail = useMutation({
    ...
    onSuccess: (url) => {
      const rv = window.location.hash.replace('#', '') || 'integrations';
      storeGmailOAuthReturnContext(rv, 'gmail');
  ...
  const exchangeCode = useMutation({
    mutationFn: async (code: string) =>
      callGmailFunction('gmail-oauth', { action: 'exchange-code', code }),
    onSuccess: () => {
  ...
  const disconnectGmail = useMutation({
    mutationFn: async (accId: string) =>
      callGmailFunction('gmail-oauth', { action: 'disconnect', account_id: accId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['gmail-accounts'] });
